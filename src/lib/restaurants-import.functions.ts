...
export type RestaurantImportPayload = {
  text?: string;
  file?: { name: string; mimeType: string; dataBase64: string };
};

export const importRestaurantReportFn = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: RestaurantImportPayload) => {
  ...
  .handler(async ({ data, context }) => {
    const mod = await import("./restaurants-import.server");
    return mod.importRestaurantReport(context.userId, data);
  });
